"""
EfficientNet v6 — Full scratch, best settings
- AdamW + CosineAnnealing + warmup 20epoch
- RandAugment + CutMix + Mixup + RandomErasing
- Drop Path 0.1
- 300 epoch
- cuda:0
"""
...
def main():
    ...
    for epoch in range(cfg['num_epochs']):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'best_weights_v6.pth')
            print(f'  ✓ best: {best_acc:.4f}')

    print(f'\n학습 완료. Best val acc: {best_acc:.4f}')

    # TorchScript export
    model.load_state_dict(torch.load('best_weights_v6.pth', map_location='cpu',
                                     weights_only=True))
    model.eval().cpu()
