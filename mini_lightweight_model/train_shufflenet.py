...
CFG = dict(
    ...
    output_pt    = 'my_model_shufflenet.pt',
)
...
class ShuffleNetV2(nn.Module):
    def __init__(self, num_classes=200, dropout=0.2):
        super().__init__()
        # width_mult = 1.5배 스펙으로 업그레이드 (파라미터 약 2.3M 확보)
...
def main():
    cfg = CFG
    model = ShuffleNetV2(200, dropout=cfg['dropout']).to(DEVICE)
    ...
    for epoch in range(cfg['num_epochs']):
        ...
        if (epoch+1) in cfg['save_epochs']:
            path = f'ckpt_shufflenet_ep{epoch+1}.pth'
            torch.save(model.state_dict(), path)
            print(f'  checkpoint: {path} (val={val_acc:.4f})')

    print(f'\ndone. Best: {best_acc:.4f}')

    model.load_state_dict(torch.load('best_shufflenet.pth', map_location='cpu', weights_only=True))
    model.eval().cpu()
