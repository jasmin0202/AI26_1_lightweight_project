...
CFG = dict(
    ...
    warmup_epoch = 10,
    rho          = 0.05,
    output_pt    = 'my_model_sam_mobilenet_v2.pt',
)
...
def main():
    ...
    for epoch in range(cfg['num_epochs']):
        ...
        if val_acc>best_acc:
            best_acc=val_acc
            torch.save(model.state_dict(),'best_sam_mobilenet.pth')
            print(f'   best: {best_acc:.4f}')
    print(f'\n done. Best: {best_acc:.4f}')
    model.load_state_dict(torch.load('best_sam_mobilenet.pth',map_location='cpu',weights_only=True))
    model.eval().cpu()
