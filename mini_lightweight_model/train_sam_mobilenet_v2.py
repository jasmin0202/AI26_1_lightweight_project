...
CFG = dict(
    ...
    output_pt    = 'my_model_mobilenet.pt',
)
...
def get_loaders(cfg):
    ...
    return (
        DataLoader(tds, batch_size=cfg['batch_size'], shuffle=True,
...
def main():
    ...
    model.load_state_dict(torch.load('best_sam_mobilenet.pth',
    ...
    def lr_lambda(e):
        total_e = e + cfg['resume_epoch']
        w = cfg['warmup_epoch']
        if total_e < w: return (total_e+1) / w
        progress = (total_e-w) / (cfg['num_epochs']-w)
    ...
    best_acc = init_val
    remaining = cfg['num_epochs'] - cfg['resume_epoch']

    for epoch in range(remaining):
        actual_epoch = epoch + cfg['resume_epoch']
        tr_loss, tr_acc = train_one_epoch(model, train_loader, optimizer, criterion, scaler, cfg)
        val_acc = evaluate(model, val_loader)
        scheduler.step()
        lr_now = optimizer.param_groups[0]['lr']
        if (epoch+1) % 10 == 0 or epoch == 0:
            print(f'[{actual_epoch+1:3d}/{cfg["num_epochs"]}] loss={tr_loss:.4f} tr={tr_acc:.4f} val={val_acc:.4f} lr={lr_now:.5f}')
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'best_mobilenet.pth')
            print(f'  best: {best_acc:.4f}')
        if (actual_epoch+1) in cfg['save_epochs']:
            path = f'ckpt_mobilenet_ep{actual_epoch+1}.pth'
            torch.save(model.state_dict(), path)
            print(f'  checkpoint saved: {path} (val={val_acc:.4f})')

    print(f'\ndone. Best: {best_acc:.4f}')

    model.load_state_dict(torch.load('best_mobilenet.pth', map_location='cpu', weights_only=True))
    model.eval().cpu()
