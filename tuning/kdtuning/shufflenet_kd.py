...
CFG = dict(
    ...
    batch_size      = 256,
    num_epochs      = 30,
    lr              = 5e-5,          # Lower LR for precise fine-tuning
    weight_decay    = 0.04,          # Stronger weight decay to prevent tr_acc from overfitting
    label_smooth    = 0.1,
    dropout         = 0.35,          # Increased dropout to suppress memorization
    num_workers     = 4,
    kd_temperature  = 4.0,
    kd_alpha        = 0.35,          # Reduced teacher ratio to balance CE and KD losses
    student_ckpt    = 'best_shufflenet_ext.pth',
    teacher_ckpt    = 'best_teacher_resnet50_v2.pth',
    output_pth      = 'best_shufflenet_kd.pth',
    output_pt       = 'my_model_shufflenet_kd.pt',
)
...
# Student: ShuffleNetV2 (width_mult=1.5)
...
def get_loaders(cfg):
    tr = transforms.Compose([
        ...
        transforms.RandomErasing(p=0.25),  # Increased erasure probability to prevent overfitting
    ])
