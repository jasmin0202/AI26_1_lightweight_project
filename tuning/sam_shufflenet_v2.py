...
DEVICE = torch.device('cuda:3') # Change to your free GPU index
    ...
    def __init__(self, in_ch, out_ch, stride=1):
        ...
        if stride == 1:
            self.branch2 = nn.Sequential(
                nn.Conv2d(in_ch // 2, branch_ch, 1, bias=False),
...
def main():
    ...
    model = ShuffleNetV2(200, dropout=0.2).to(DEVICE)
    model.load_state_dict(torch.load('best_shufflenet.pth', map_location=DEVICE, weights_only=True))

    epochs = 20
    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
    base_opt = optim.AdamW
    optimizer = SAM(model.parameters(), base_opt, rho=0.05, lr=5e-5, weight_decay=0.01)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for imgs, labels in val_loader:
                ...
                tot += imgs.size(0)
        val_acc = (cor / tot) * 100
        print(f"[ShuffleNetV2] Epoch {epoch+1}/{epochs} | Val Acc: {val_acc:.2f}%", flush=True)
        
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'best_sam_tuned_shufflenet.pth')
