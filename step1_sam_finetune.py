...
def train_one_epoch_sam(model,loader,optimizer,criterion,cfg):
...
def finetune(name, model, ckpt_path, save_path, train_loader, val_loader, cfg):
    print(f'\n--- SAM fine-tune: {name} ---')
    model.load_state_dict(torch.load(ckpt_path, map_location=DEVICE, weights_only=True))
    init_val = evaluate(model, val_loader)
    print(f'init val: {init_val:.4f}')

    criterion = LabelSmoothingCE(200, cfg['label_smooth'])
    optimizer = SAM(model.parameters(), optim.AdamW,
                    rho=0.05, lr=cfg['lr'], weight_decay=cfg['weight_decay'])
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer.base_optimizer, T_max=cfg['num_epochs'], eta_min=1e-7)

    best_acc = init_val
    for epoch in range(cfg['num_epochs']):
        tr_loss, tr_acc = train_one_epoch_sam(model, train_loader, optimizer, criterion, cfg)
        val_acc = evaluate(model, val_loader)
        scheduler.step()
        print(f'[{epoch+1:2d}/{cfg["num_epochs"]}] loss={tr_loss:.4f} tr={tr_acc:.4f} val={val_acc:.4f}')
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), save_path)
            print(f'  best: {best_acc:.4f}')

    print(f'--- {name} done. Best: {best_acc:.4f} ---')
    return best_acc
...
def main():
    ...
    m1 = TinyResNet18(200, 0.48, cfg['dropout']).to(DEVICE)
    finetune('ResNet', m1, 'best_weights_auto_resnet.pth',
             'sam_ft_resnet.pth', train_loader, val_loader, cfg)

    m2 = get_mobilenet(200).to(DEVICE)
    finetune('MobileNet', m2, 'best_weights_auto_mobilenet.pth',
             'sam_ft_mobilenet.pth', train_loader, val_loader, cfg)

    print('\n완료. sam_ft_resnet.pth / sam_ft_mobilenet.pth 저장됨')
