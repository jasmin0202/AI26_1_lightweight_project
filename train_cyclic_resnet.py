...
CFG = dict(
    ...
    output_pt    = 'my_model_cyclic_resnet.pt',
)
...
def main():
    ...
    for epoch in range(cfg['num_epochs']):
        ...
        if val_acc>best_acc:
            best_acc=val_acc
            torch.save(model.state_dict(),'best_cyclic_resnet.pth')
            print(f'  best: {best_acc:.4f}')
        if (epoch+1) in cfg['save_epochs']:
            path=f'ckpt_cyclic_resnet_ep{epoch+1}.pth'
            torch.save(model.state_dict(),path)
            print(f'  checkpoint: {path} (val={val_acc:.4f})')

    print(f'\ndone. Best: {best_acc:.4f}')

    model.load_state_dict(torch.load('best_cyclic_resnet.pth',map_location='cpu',weights_only=True))
    model.eval().cpu()
